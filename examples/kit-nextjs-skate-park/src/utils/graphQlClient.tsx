...
console.log('[ContentTypeListing] edgeUrl =', scConfig.api.edge.edgeUrl);
console.log('[ContentTypeListing] contextId =', scConfig.api.edge.contextId);
...
export const getContentGraphQlClient = (): GraphQLRequestClient => {
  const contextId = scConfig.api.edge.contextId;
  const endpoint = `${scConfig.api.edge.edgeUrl}/v1/content/api/graphql/v1?sitecoreContextId=${contextId}`;

  return new GraphQLRequestClient(endpoint, {
    // contextId auth travels in the query string; apiKey header may be ignored/optional
  });
};